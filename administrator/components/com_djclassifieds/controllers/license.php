...
class DjclassifiedsControllerLicense extends JControllerLegacy {
	...
	public function edit(){
		...
		if(!$key){
			...
			echo '<div class="lic_srow2">'.JText::_('COM_DJCLASSIFIEDS_DJLIC_YOU_CAN_GET').' <a target="_blank" href="http://www.dj-extensions.com">DJ-EXTENSIONS.COM</a></div>';
			echo '</div>';
	...
	function update_list(){
		...
		$query = "SELECT name, type, element, manifest_cache "				
				."FROM #__extensions WHERE (element LIKE '%".$ext_name."%' AND folder!='djmediatools' AND folder!='djreviews') OR element LIKE '%".$ext_name2."%' OR folder='djclassifieds' "
						."ORDER BY type ";
		$db->setQuery($query);
